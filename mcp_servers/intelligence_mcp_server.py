...
from anomaly_engine import detect_transaction_anomalies as engine_detect_anomalies
from debt_optimizer import optimize_debt as engine_optimize_debt, all_strategy_rankings
from digital_twin import simulate_financial_scenario as engine_simulate_financial_scenario
from forecast_engine import forecast_cash_flow as engine_forecast_cash_flow, forecast_spending as engine_forecast_spending, aggregate_monthly_cash
from goal_engine import plan_financial_goal as engine_plan_financial_goal
...
def plan_financial_goal(target_amount: float, current_saved_amount: float, months_remaining: int,
    ...
    return _json(engine_plan_financial_goal(target_amount, current_saved_amount, months_remaining,
...
def optimize_debt(monthly_income: float, existing_emi: float = 0.0) -> str:
    ...
    recs = engine_optimize_debt(items, monthly_income, existing_emi)
...
def simulate_financial_scenario(salary_change_percentage: float = 0.0,
    ...
    return _json(engine_simulate_financial_scenario(snapshot, scenario).model_dump())
